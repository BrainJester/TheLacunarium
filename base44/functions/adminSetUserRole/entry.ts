...
export default async function (req: Request): Promise<Response> {
  try {
    ...
    if (body.role !== 'user' && body.role !== 'tester') fail(400, 'Choose user or tester.');
    ...
    if (profile.user_id === user.id) fail(400, 'The owner role cannot be changed here.');
    await base44.asServiceRole.entities.Profile.update(profile.id, { site_role: body.role });
